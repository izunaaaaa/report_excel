...
void Dialog::dragMoveEvent(QDragEnterEvent *event) // ????? ?????
{
    event->acceptProposedAction();
}
...
void Dialog::navi_cam() // 같은 시간대의 네비캠 이미지 ( 1장 )
{
    ...
    for ( int j = 0; j < navi_Filelist.size(); j++)
    {
        ...
        else if(navi_Filelist.at(j).contains( back_img_time))
        {
            navi_idx = j;
            navi_reader.setFileName(navi_Filelist.at(navi_idx-15));
...
//            if (navi_idx + 70< navi_Filelist.size())
//            {
//                navi_reader.setFileName(navi_Filelist.at(navi_idx+70));
//                navi_img_2 = navi_reader.read();
//            }
//            else{
//                navi_reader.setFileName(navi_Filelist.at(navi_Filelist.size()-1));
//                navi_img_2 = navi_reader.read();
//            }
//            qDebug() << toKor("네비캠 Ok") << navi_idx;
//            return;
